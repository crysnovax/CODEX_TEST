// ── 𝗖𝗢𝗗𝗘𝗫 𝐀𝐈 V2 SMART SEARCH ENGINE ──
...
async function searchDefinition(query) {
    try {
        ...
        const { data } = await axios.get(url, {
            headers: { 'User-Agent': '✦𝗖𝗢𝗗𝗘𝗫 𝐀𝐈/2.0' }
        });
    ...
    try {
        ...
        try {
            ...
            const { data } = await axios.get(wikiUrl, {
                headers: { 'User-Agent': '✦𝗖𝗢𝗗𝗘𝗫 𝐨𝐟𝐟𝐢𝐜𝐢𝐚𝐥/2.0' }
            });
